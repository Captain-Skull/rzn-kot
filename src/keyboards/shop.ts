...
export async function shopKeyboard(
  ...
  for (const p of products) {
    ...
    if (inCart > 0) {
        keyboard
            ...
            .text('Удалить', `remove-from-cart_${p.label}_${p.price}_${category}`)
            .row();
    } else {
        keyboard
            .text(text, `add-to-cart_${p.label}_${p.price}_${category}`)
  ...
  if (category === ProductCategory.CODES) {
    ...
    keyboard
      .text("🛒 Купить по ID", `cart_buy-with-id_${category}`)
      .row();
  }

  keyboard
    .text("🗑 Очистить корзину", `cart_clear_${category}`)
    .row()
    .text("🔙 В главное меню", "return");
